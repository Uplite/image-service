...
func (m *mockReader) ReadOne(ctx context.Context, key string) (string, error) {
	return config.GetCloudFrontURL() + "/" + "image_1", nil
}

func (m *mockReader) ReadMany(ctx context.Context, prefix string) ([]string, error) {
	v1 := config.GetCloudFrontURL() + "/" + "image_1"
	v2 := config.GetCloudFrontURL() + "/" + "image_2"
...
func TestReaderServer(t *testing.T) {
	...
	client := pb.NewImageServiceReaderClient(conn)

	ctx, cancel := context.WithTimeout(context.Background(), 1*time.Second)
	defer cancel()

	t.Run("should get one", func(t *testing.T) {
		res, err := client.GetOne(ctx, &pb.GetOneRequest{Key: "image_1"})
		assert.NoError(t, err)
		assert.Equal(t, res.GetUrl(), config.GetCloudFrontURL()+"/"+"image_1")
	})
	...
	t.Run("should get many", func(t *testing.T) {
		...
		assert.Equal(t, res.GetUrls()[0], config.GetCloudFrontURL()+"/"+"image_1")
		assert.Equal(t, res.GetUrls()[1], config.GetCloudFrontURL()+"/"+"image_2")
	})
}
